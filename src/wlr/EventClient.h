...
class EventClient
{
 private:
  std::vector<std::move_only_function<void()>>
    events_handle_cancels_;       // Stores one lambda per event handle that will cancel the event callback if invoked.
  std::vector<EventsContainer*>
    registered_event_containers_; // A list of EventsContainer objects for which register_event(events_container, ...) was called.
 ...
 public:
  ...
  {
    static_assert(std::is_same_v<typename EVENTS_CONTAINER::wlr_events_container_type,
                    typename wl::EventInfo<EVENT_TYPE::signal_enum>::wlr_events_container_type>,
    ...
    DoutEntering(dc::events,
      "EventClient::register_event(" << print_type<EVENTS_CONTAINER&> << " [@" << (void*)&events_container << "], " << "void ("
                                     << print_type<CLIENT> << "::*)(" << print_type<EVENT_TYPE> << " (with EventData "
    ...
    auto iter =
      std::find(registered_event_containers_.begin(), registered_event_containers_.end(), static_cast<EventsContainer*>(&events_container));
